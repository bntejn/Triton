 ...
 */
    ...
    class API {
      ...
      public:
        ...
        //! [**taint api**] - Taints an address with a tag. Returns TAINTED if the address has been tainted correctly. Otherwise it returns the last defined state.
        bool taintMemory(triton::uint64 addr, triton::engines::taint::TagType tag);
        ...
        bool taintMemory(const triton::arch::MemoryAccess& mem, triton::engines::taint::TagType tag);
        ...
        bool taintRegister(const triton::arch::Register& reg, triton::engines::taint::TagType tag);
        ...
        std::vector<triton::engines::taint::TagType*> getTags(const triton::arch::Register& reg);

        //! [**taint api**] - Retrieve the tags assigned to a MemoryAccess
        std::vector<triton::engines::taint::TagType*> getTags(const triton::arch::MemoryAccess& mem);

        //! [**taint api**] - Retrieve the tags assigned to a memory address
        std::vector<triton::engines::taint::TagType*> getTags(const triton::uint64 addr);
    };
